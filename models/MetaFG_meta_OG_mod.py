...
from .LargeTransformerEncoderLayer import LargerTransformerEncoder
from .ImprovedResBlock import ImprovedResBlock
...
#             assert len(meta_dims)==extra_token_num-1
            for ind,meta_dim in enumerate(meta_dims):
                ...
                elif meta_encoding == "iresnorm":
                    meta_head_1 = nn.Sequential(
                        nn.Linear(meta_dim, attn_embed_dims[1]),
                        nn.GELU(),
                        nn.LayerNorm(attn_embed_dims[1]),
                        ImprovedResBlock(attn_embed_dims[1], expansion_factor=4, dropout=0.1),
                    ) if meta_dim > 0 else nn.Identity()

                elif meta_encoding == "mlp":
                    meta_head_1 = nn.Sequential(
                        nn.Linear(meta_dim, attn_embed_dims[0]),  # Keep this for input transformation
                        MLPLayer(attn_embed_dims[0])  # No need for extra ReLU/LayerNorm, handled inside MLP
                    ) if meta_dim > 0 else nn.Identity()

                elif meta_encoding == "transformer":
                    meta_head_1 = nn.Sequential(
                        nn.Linear(meta_dim, attn_embed_dims[0]),  # Ensure correct input dimension
                        TransformerEncoderLayer(attn_embed_dims[0])  # Transformer handles normalization internally
                    ) if meta_dim > 0 else nn.Identity()
                    
                elif meta_encoding == "large_transformer":
                    meta_head_1 = nn.Sequential(
                        nn.Linear(meta_dim, attn_embed_dims[0]),  # Ensure correct input dimension
                        LargerTransformerEncoder(attn_embed_dims[0])  # Transformer handles normalization internally
                    ) if meta_dim > 0 else nn.Identity()
                    
                elif meta_encoding == "deepmlp":
                    meta_head_1 = nn.Sequential(
                        nn.Linear(meta_dim, attn_embed_dims[0]),  # Ensure correct input dimension
                        DeepMLP(attn_embed_dims[0])  # MLP handles its own normalization
                    ) if meta_dim > 0 else nn.Identity()
                ...
                if meta_encoding == "resnorm":
                    ...
                    meta_head_2 = nn.Sequential(
                        nn.Linear(meta_dim, attn_embed_dims[1]),  # Project metadata into correct dim
                        MLPLayer(attn_embed_dims[1])  # MLP handles its own activation and normalization
                    ) if meta_dim > 0 else nn.Identity()

                elif meta_encoding == "transformer":
                    meta_head_2 = nn.Sequential(
                        nn.Linear(meta_dim, attn_embed_dims[1]),  # Ensure correct input shape
                        TransformerEncoderLayer(attn_embed_dims[1])  # Transformer applies its own normalization
                    ) if meta_dim > 0 else nn.Identity()
                
                elif meta_encoding == "large_transformer":
                    meta_head_2 = nn.Sequential(
                        nn.Linear(meta_dim, attn_embed_dims[1]),  # Ensure correct input shape
                        LargerTransformerEncoder(attn_embed_dims[1])  # Transformer applies its own normalization
                    ) if meta_dim > 0 else nn.Identity()

                elif meta_encoding == "deepmlp":
                    meta_head_2 = nn.Sequential(
                        nn.Linear(meta_dim, attn_embed_dims[1]),  # Ensure correct input shape
                        DeepMLP(attn_embed_dims[1])  # MLP applies its own normalization
                    ) if meta_dim > 0 else nn.Identity()
    ...
    def forward_features(self,x,meta=None):
        ...
        if self.add_meta:
            ...
            for ind,cur_meta in enumerate(metas):
                ...
                # if isinstance(meta_head_1, TransformerEncoderLayer):
                #     meta_1 = meta_1.unsqueeze(1)  # Add sequence length dimension for Transformer
                #     meta_1 = meta_head_1(meta_1)  # Apply Transformer
                #     meta_1 = meta_1.squeeze(1)  # Remove extra dimension
                # else:
                #     meta_1 = meta_head_1(cur_meta)

                # if isinstance(meta_head_2, TransformerEncoderLayer):
                #     meta_2 = meta_2.unsqueeze(1)  # Add sequence length dimension for Transformer
                #     meta_2 = meta_head_2(meta_2)  # Apply Transformer
                #     meta_2 = meta_2.squeeze(1)  # Remove extra dimension
                # else:
                #     meta_2 = meta_head_1(cur_meta)
...
if __name__ == "__main__":
    ...
    model = MetaFG_meta_2(meta_encoding="transformer")
    # model = MetaFG_Meta(meta_encoding="mlp")
